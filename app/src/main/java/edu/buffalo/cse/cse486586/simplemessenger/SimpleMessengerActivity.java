...
public class SimpleMessengerActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            new ServerTask().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, serverSocket); // it is thread_pool because it requires threads to run in parallel i.e one server thread to listen and one client thread to listen on
        } catch (IOException e) {
            /*
        ...
        editText.setOnKeyListener(new OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if ((event.getAction() == KeyEvent.ACTION_DOWN) &&
                     ...
                     * http://developer.android.com/reference/android/os/AsyncTask.html
                     */
                    new ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, msg, myPort); /// it creates a thread
                    //serial executer-  one thread at a time ,
                    // thread_pool_eecuter - all threads will run in parralel-
                    return true;
                }
                return false;
            }
    ...
    private class ServerTask extends AsyncTask<ServerSocket, String, Void> {
        ...
        protected Void doInBackground(ServerSocket... sockets) {
            ...
            try {
                while (true) {
                    Socket socket = serverSocket.accept(); // to accept the connection from the client.

                    DataInputStream disin = new DataInputStream(socket.getInputStream()); // input stream to get messages from the client
                    String msgReceived = disin.readUTF(); // finally reading the message and storing it in String format
                    publishProgress(msgReceived); // publishing the message on the screen // This method will pass the string to onProgressUpdate


                    DataOutputStream disout = new DataOutputStream(socket.getOutputStream()); // output stream to send acknowledgment to the client
                    disout.writeUTF("ACKNOWLEDGE"); //writng the acknowledge message to the stream and sending it

                    disin.close();//closing the input stream
                    disout.close(); // closing the ouput stream
    ...
    private class ClientTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... msgs) {
            try {
                ...
                DataOutputStream disout =new DataOutputStream(socket.getOutputStream()); //output stream to get send messages to the server
                disout.writeUTF(msgToSend);// writing the message to the output stream which will send this message to the server


                DataInputStream disin=new DataInputStream(socket.getInputStream()); //input stream to read the "acknowledge" from the server that it has received the message
                disin.readUTF(); //finally reading the acknowldege from the server

                disin.close();  // closing the input stream
                disout.close(); // closing the output stream
                socket.close(); // closing the socket
...
//socket.close runs simulataneously with the sent so other side is not receiving the actual message as it gets socket.close() also.